# processors/shopify_brian.py
from .shopify_common import process as shopify_process

def process(df):
    """Shopify processing for Brian with additional customization"""
    # Make a copy to avoid modifying the original
    df_copy = df.copy()
    
    # Clean up Shipping Zip
    df_copy['Shipping Zip'] = df_copy['Shipping Zip'].str.replace("^'", "", regex=True)
    
    # First apply common Shopify processing
    df_processed = shopify_process(df_copy)
    ...
    mask_la_ms = df_processed['State'].isin(['LA', 'MS'])  # No zip filtering for LA, MS, and PA
    mask_al_fl = (df_processed['State'].isin(['AL', 'FL'])) & (df_processed['Zip'].isin(zip))  # Zip filtering for AL and FL
    
    # Combine masks with OR operator
    combined_mask = mask_la_ms | mask_al_fl
    
    # Apply the filter AFTER processing
    df_filtered = df_processed[combined_mask]

...
def process(df):
    ...
    mask_la_ms = df['Shipping State'].isin(['LA', 'MS'])  # No zip filtering for LA, MS, and PA
    mask_al_fl = (df['Shipping State'].isin(['AL', 'FL'])) & (df['Shipping Zip/Postal Code'].isin(zip))  # Zip filtering for AL and FL
